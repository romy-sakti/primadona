...
class UserRepository extends Repository
{
    ...
    {
        $users = $this->model->with(['roles'])->orderBy('name')->get();
        return $users;
    }
    ...
    {
        // Jika roles adalah single value (string atau integer)
        if (!is_array($roles)) {
            $roles = [$roles];
        }

        // Konversi ID role menjadi nama role
        $roleNames = collect($roles)->map(function ($roleId) {
        ...
        return $user->syncRoles($roleNames);
    }
    ...
    {
        // Tambahkan pengisian email_verified_at dengan waktu saat ini
